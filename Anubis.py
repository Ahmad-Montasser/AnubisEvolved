...
import Python_Coloring as pyc

import FastExecute as fep
def serial_ports():
...
class text_widget(QWidget):
    def __init__(self,fastExecute):
        super().__init__()
        self.itUI(fastExecute)
    def itUI(self,fastExecute):
        global text
        text = QTextEdit()
        if fastExecute==0:
            pyc.PythonHighlighter(text)
        else:
            pyc.PythonHighlighter(text)
        hbox = QHBoxLayout()
        hbox.addWidget(text)
...
#        Port.addAction(Port_Action)
        ...
        #Add fast execution menu and bind it to the execute function
        FE_Action =QAction("Fast Execute Python",self)
        FE_Action.triggered.connect(self.execute)
        filemenu.addAction(Save_Action)
        filemenu.addAction(Close_Action)
        filemenu.addAction(Open_Action)
        FE.addAction(FE_Action)
        ...
        widget = Widget(self.fastExecute)
